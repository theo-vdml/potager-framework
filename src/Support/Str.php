...
class Str
{
    ...
    {
        $string = iconv('UTF-8', 'ASCII//TRANSLIT//IGNORE', $string); // 👈 remplacer les accents par leur equivalents ASCII
        $string = preg_replace('/([a-z])([A-Z])/', '$1 $2', $string); // 👈 camelCase -> camel Case
        $string = preg_replace('/[\s_\-]+/', ' ', $string); // 👈 espace, undersore, tirets -> espace unique
        $string = strtolower(trim($string)); // 👈 tout en minuscule + retirer leading spaces
        return $string;
    }
